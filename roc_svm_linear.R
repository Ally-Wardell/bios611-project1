# Program Name: roc_svm_linear.R
# Purpose: Machine learning SVM model with a linear kernel : ROC CURVE
...
best_thres_data <- 
  data.frame(coords(roc_obj_svml, x="best", best.method = c("youden")))

# Plot ROC curve
svml_roc_plot = ggroc(roc_obj_svml)+
  geom_point(
  ...
  labs(title = 
         paste0("ROC curve for predicting heart disease using SVM Linear Kernel, on test set\nAUC = ", round(auc(roc_obj_svml),2)))+
  theme_classic()


png("figures/roc_svm_linear.png")
plot(svml_roc_plot)
dev.off()